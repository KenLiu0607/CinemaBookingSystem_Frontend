        ...
        <el-card class="booking-card" shadow="hover">
          <div class="movie-header">
            ...
            <div class="movie-meta">
              ...
              <p class="movie-info">
                {{ activeMovie.runtime }} min / {{ activeMovie.language }}
              </p>
            </div>
          </div>
      ...
      <el-col :xs="24" :lg="14">
        <el-card class="booking-card" shadow="hover">
          ...
          <div class="action-bar">
            <el-button type="primary" :disabled="!canSubmit" @click="handleBooking">
              Confirm Booking
            </el-button>
            <span class="available-info">
              Seats left: {{ availableSeatCount }}
            </span>
          </div>
        </el-card>
      </el-col>
...
const activeMovie = computed(
  () => movies.value.find((movie) => movie.id === formState.movieId) ?? movies.value[0]
);
...
const totalPrice = computed(() =>
  activeMovie.value ? activeMovie.value.price * selectedSeats.value.length : 0
);
...
const canSubmit = computed(
  () => !!formState.movieId && !!formState.showtime && selectedSeats.value.length > 0
);
...
const handleBooking = () => {
  ...
  ElMessage.success(
    `Booking confirmed: ${activeMovie.value.title} on ${formattedDate.value} at ${formState.showtime}. Seats: ${selectedSeatsArray.value.join(
      ", "
    )}`
  );
};
